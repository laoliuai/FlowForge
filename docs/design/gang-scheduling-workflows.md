...
3. 调度器按 Gang 原则调度：
   ...
   - 引入优先级与队列（按租户配额权重、workflow 优先级）。
   - 对等待过久的 Gang 触发超时回退策略（降级并发、拆分批次或让位）。

### 公平性与饥饿避免
- **租户公平**：调度器基于配额权重进行加权轮询，避免大租户独占资源。
- **小任务保护**：对小规模 Gang 提供快速通道，避免长时间排队导致 SLA 失效。
- **等待超时**：超过阈值的 Gang 可进入降级模式或触发告警。

### 资源预占与原子性
- 调度器在创建 Pod 前进行 **配额预占 (reservation)**，确保多 Pod 组不会被竞态抢占。
- 预占成功后进入 Pod 创建；若创建失败或超时，释放预占资源。
- 预占使用唯一 `reservation_id`，确保重试与释放操作幂等。
...
### Schema 版本与校验
- DAG 描述包含 `version` 字段，服务端根据版本做兼容解析。
- 服务端校验：环路检测、依赖合法性、资源请求有效性与配额预检查。
...
- **K8s 调度器感知不足**：应用层预占与 K8s 实际可用资源可能不一致，存在 “逻辑足够、物理不足” 的 Pending 风险。

## 调度生态集成建议
- **Volcano/Kueue**：优先评估与成熟批处理调度器集成，通过 PodGroup/Queue 把 Gang 语义下沉到调度层，避免应用层死锁。
- **混合模式**：若保持自研预占逻辑，可引入容量缓存 + 预检（Node resources snapshot）与超时回收策略，降低 Pending 风险。
